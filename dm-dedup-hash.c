				...
				struct bio *bio, char *hash)
{
	...
	sg_init_table(&sg, 1);
	__bio_for_each_segment(bvec, bio, iter, bio->bi_iter) {    //amit
...
int compute_page_hash(struct hash_desc_table *desc_table,
				struct page* page, char *hash)
{
	struct scatterlist sg;
	int ret = 0;
	unsigned long slot;
